...
export class RegisterComponent implements OnInit {
  ...
  onRegisterUser(form2: NgForm) {
    if (!form2.valid) {
      this.isEmpty = true;
      if (form2.controls.city.value == "") this.warning = "El campo de la ciudad es obligatorio";
      else this.warning = "Por favor llene todos los campos requeridos";
    } else {
      this.isLoading = true;
      this.isEmpty = false;
      this.userRegisterData = {
      ...
      this.authService.registerUser(this.userRegisterData).subscribe(
        ...
        err => {
          ...
          if (err.status) {
            this.isEmpty = true;
            this.warning = "No se pudo registrar al usuario.";
